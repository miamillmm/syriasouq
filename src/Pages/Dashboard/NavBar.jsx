...
const Navbar = () => {
  ...
  return (
    <nav className="p-4">
      <div className="flex items-center gap-6 mb-5 group relative w-fit">
        <div className="w-10 h-10 rounded-full border-2 border-gray-400 bg-gray-200 shadow-xl flex items-center justify-center cursor-pointer">
          <span className="font-bold ">
            {user?.username?.slice(0, 1).toUpperCase()}
          </span>
        </div>

        <div className="absolute top-10 left-0 min-w-60 p-3 rounded-lg shadow-lg bg-slate-200 z-[99999] hidden group-hover:flex flex-col gap-2 transition-all">
          <p>{user?.email}</p>
          <hr />
